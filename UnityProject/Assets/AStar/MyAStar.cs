    ...
    void Init ()
    {
        ...
        for (int i = 0; i < x; i++) {
            for (int j = 0; j < y; j++) {
                ...
                GameObject item = (GameObject)Instantiate (reference,new Vector3 (i, 0, j) + startPos,Quaternion.identity);
                item.GetComponent<Reference>().x = i;
                item.GetComponent<Reference>().y = j;
                objs [i, j] = item;
            }
        }
    }
